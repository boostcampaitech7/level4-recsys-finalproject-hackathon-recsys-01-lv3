...
def load_data_lazy(base_path: str, preprocess: bool = False):
    ...
    if preprocess:
        log_data = preprocess_log_data(log_data, filter_session, user_data)
...
def main():
    ...
    log_data, user_data, item_data, cat_table, cat_table_kr, brand_table, filter_session = load_data_lazy(base_path, preprocess=True)

    # 최종적으로 데이터를 물리적 DataFrame으로 변환
    log_data_processed = log_data.collect()
    print(log_data_processed.head())
