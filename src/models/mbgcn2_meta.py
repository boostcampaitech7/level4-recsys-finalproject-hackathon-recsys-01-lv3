...
class ModelBase(nn.Module):
    def __init__(self, args: argparse.Namespace, trainset, device: torch.device):
        ...
        # User and item meta information
        self.user_meta = trainset.user_meta  # User meta information as a tensor
        self.item_meta = trainset.item_meta  # Item meta information as a tensor
        self.user_meta_mlp = nn.Linear(self.user_meta.shape[1], self.embed_size)
        self.item_meta_mlp = nn.Linear(self.item_meta.shape[1], self.embed_size)
...
class MF(ModelBase):
    ...
    def propagate(self, task: str = 'train'):
        # Apply meta-information embeddings
        user_meta_emb = self.user_meta_mlp(self.user_meta)
        item_meta_emb = self.item_meta_mlp(self.item_meta)

        # Combine ID and meta embeddings
        user_feature = self.user_embedding + user_meta_emb
        item_feature = self.item_embedding + item_meta_emb
